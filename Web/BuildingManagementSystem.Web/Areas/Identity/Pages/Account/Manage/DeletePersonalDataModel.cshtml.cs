namespace BuildingManagementSystem.Web.Areas.Identity.Pages.Account.Manage
{
    ...
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Възникна неочаквана грешка при изтриване на потребител с идентификатор ID: '{userId}'.");
            }

            await this.signInManager.SignOutAsync();

            this.logger.LogInformation("Потребител с идентификатор ID: '{UserId}' се изтри.", userId);
